...
def _combine_fecha_hora(fecha: pd.Series, hora: pd.Series) -> tuple[pd.Series, pd.Series]:
    """
...
# ==============================================================================
# Carga principal (misma idea que tu pipeline)
# ==============================================================================
...
def load_entradas_excel(filepath: Union[str, Path]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    Lee el Excel histórico de entradas:
      - Hoja: "Entrada Inv." si existe
      - skiprows=3
      - limpia Unnamed / columnas vacías
      - estandariza y filtra filas no útiles
    Retorna:
      df_ok: dataset limpio + reglas (calidad/doc_prefix)
      df_bad: rechazados con columna 'motivo'
    """
    path = Path(filepath)
    if not path.exists():
    ...
    if "Entrada Inv." in sheets:
        df_raw = pd.read_excel(path, sheet_name="Entrada Inv.", skiprows=3, dtype={"CLAVE": str})
    else:
        # fallback: primera hoja
        df_raw = pd.read_excel(path, sheet_name=sheets[0] if sheets else 0, dtype={"CLAVE": str})

    df_raw = _drop_unnamed(df_raw)
    ...
    if "producto" not in df.columns:
        # si viene con otro nombre típico, lo intentamos rescatar
        for alt in ["comentario", "NOMBRE  DEL PRODUCTO", "NOMBRE DEL PRODUCTO"]:
    ...
    # Motivos de rechazo (limpieza de filas no importantes)
    ...
    # Reglas de negocio: calidad + doc_prefix
