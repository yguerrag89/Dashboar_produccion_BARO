...
def _ensure_base(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Columnas objetivo (mínimas):
      ...
      - calidad (PRIMERA/SEGUNDA/OTROS) (derivable)
      - doc_prefix (derivable)
    """
    out = df.copy()
    ...
    if "producto" not in out.columns:
        # intentos de rescate
        for alt in ["comentario", "NOMBRE  DEL PRODUCTO", "NOMBRE DEL PRODUCTO", "DESCRIPCION"]:
    ...
    if "documento_ref" not in out.columns:
        for alt in ["ALMACEN Y/O PEDIDO (SERIE Y FOLIO)", "documento"]:
            if alt in out.columns:
    ...
    # ---- reglas (calidad / doc_prefix) si faltan ----
    if "calidad" not in out.columns or "doc_prefix" not in out.columns:
        out = apply_rules(out, producto_col="producto", documento_ref_col="documento_ref")

    # ---- calendario ----
    out["anio"] = out["dia"].dt.year
    out["mes"] = out["dia"].dt.to_period("M").dt.to_timestamp()  # primer día del mes (Timestamp)
