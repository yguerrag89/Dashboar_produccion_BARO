...
include_otros = st.sidebar.toggle(
    "Incluir OTROS",
    value=False,
    help="Por defecto, nos enfocamos en PRIMERA/SEGUNDA para analizar calidad.",
)
...
# ==============================================================================
# ranking para que el selector sea útil (los más movidos arriba)
sku_rank = (
    bf.groupby("sku", as_index=False)["piezas"].sum()
    .sort_values("piezas", ascending=False)
)
sku_list = sku_rank["sku"].astype(str).tolist()

sku_selected = st.sidebar.selectbox("SKU", options=sku_list, index=0)
...
st.caption("Reglas: PRIMERA = BISA+IMU+POP | SEGUNDA = SANSON | OTROS = resto")
...
g = (
    df_sku.groupby(["mes", "calidad"], as_index=False)["piezas"].sum()
)
piv = g.pivot_table(index="mes", columns="calidad", values="piezas", aggfunc="sum", fill_value=0).reset_index()
...
# ==============================================================================
# 3) Corte por doc_prefix
# ==============================================================================
st.markdown("---")
st.subheader("Corte por doc_prefix")
...
ax3.barh(p["doc_prefix"].astype(str), p["piezas_total"])
ax3.set_xlabel("Piezas")
ax3.set_title(f"SKU {sku_selected} - Top {topn} doc_prefix por volumen")
